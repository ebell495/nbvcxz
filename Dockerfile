...
RUN mkdir ./fuzz/deps && find ./target -name "nbvcxz*.0.jar" -exec cp {} "./fuzz/deps/nbvcxz.jar" \; && python3 fuzz/generate_classpath.py > fuzz/src/Manifest.txt
...
RUN javac -cp "../deps/*" fuzz.java && jar cfm fuzz.jar Manifest.txt fuzz.class && chmod u+x fuzz.jar && cp fuzz.jar /nbvcxz/fuzz/deps
